...
if command -v bankr &> /dev/null; then
  ...
  echo "  bankr login"
  echo "  bankr prompt \"Send $AMOUNT USDC to AsterPay EUR off-ramp for IBAN $IBAN. Use AsterPay's x402 settlement API at https://x402-api-production-ba87.up.railway.app to convert USDC to EUR via SEPA Instant.\""
  echo ""
  echo "Option 2 - Use Bankr REST API:"
  echo "  curl -X POST https://api.bankr.bot/agent/prompt \\"
  echo "    -H \"X-API-Key: \$BANKR_API_KEY\" \\"
  echo "    -H \"Content-Type: application/json\" \\"
  echo "    -d '{\"prompt\": \"Send $AMOUNT USDC to AsterPay EUR off-ramp for IBAN $IBAN. Use AsterPay x402 settlement API at https://x402-api-production-ba87.up.railway.app to convert USDC to EUR via SEPA Instant.\"}'"
  echo ""
